...
//
// Enforcement points (mirrored from client/):
//   - ingress data path in run.go, after control-frame dispatch
//   - case 'R' relay handler in overlay.go, requesting + forwarding sessions
//   - the return-path relay in run.go's decrypt loop
//   - egress unicast and the discovery flood in run.go
