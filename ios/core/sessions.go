...
const (
	// Untyped (not `byte`) on purpose: gomobile/gobind cannot bind exported
	// byte-typed constants and fails the iOS/Android build ("unsupported const
	// type byte"). Untyped integer constants convert to byte automatically in
	// every use below (comparisons, []byte{...} literals, frame[0] = …), so the
	// wire protocol is unchanged while gobind is satisfied.
	PktMsg1        = 0x01
	PktMsg2        = 0x02
	PktMsg3        = 0x03
	PktData        = 0x04
...
// sharing the socket). Unexported so gomobile/gobind doesn't try to bind a
// scalar-byte parameter (which it emits as an invalid C `byte` type).
...
// in ip_learning never favoured it. Checking our own attached subnets first
// covers those cases; the RFC1918 test stays as a fallback for addresses on a
// LAN we reach via another hop.
...
// deliverPacket routes an inbound packet to the right handshake or session.
// typ is the first byte of the UDP datagram; body is everything after it.
// Returns true if the handshake layer consumed the packet; false means the
// caller should treat it as a data frame and decrypt against the established
// session. Unexported so gomobile/gobind doesn't bind its scalar-byte `typ`
// parameter (which it emits as an invalid C `byte` type).
func (t *SessionTable) deliverPacket(raddr *net.UDPAddr, typ byte, body []byte, kp keypair, psk []byte) bool {
	key := raddr.String()
