...
// revokedIPs is a data-plane block list of overlay IPs of revoked keys, so a
// revoked node is dropped even over a relay path (not just refused at the
// handshake) — you can't even ping it.
var (
...
// the peer's static key so a re-admit clears the exact IP that was blocked.
