...
type ClientConfig struct {
	...
	// admin dashboard). Optional; also settable remotely by an admin provision.
	...
	// clamped 5..120 — short enough to survive aggressive consumer/carrier
	// NATs whose UDP mappings expire in 15-30s.
...
func stunQueryOnSocket(conn *net.UDPConn, stunServer string, timeout time.Duration) (string, error) {
	...
	// caller; now that NAT probing also runs periodically, the two would
	// collide on a regular cadence and each collision costs a real STUN
	// timeout. Serialising costs nothing: these are infrequent and short.
	stunQueryMu.Lock()
...
var (
	...
	// portPredictionForced records the config setting, so the UI can tell
...
// ipv6Enabled gates the dual-stack transport. ON by default.
...
// It used to be started only when port_prediction was configured on, which the
// mobile app never sets, so a symmetric NAT was never probed, never logged and
// never shown. Joining a network behind such a router therefore looked like
...
// only candidates with any chance.
...
func myConnectCandidates() string {
	...
	// 2. IPv6: no NAT at all, so a v6-capable peer reaches us directly.
	if myUDPPort > 0 {
		tier = 0
	...
	// 3. The observed public endpoint. (No NAT-PMP step here: the mobile core
	// has no portmap.go — an iOS/Android device cannot usefully hold a router
	// pinhole open across sleep and network changes.)
	if m.ip == "" {
	...
	// 4. Our public IP paired with our OWN LISTEN PORT.
	//
	// STUN can only report the port our NAT chose for the packet we sent TO
	// STUN. That is right behind a plain home router and wrong whenever the
	// real inbound path was configured rather than negotiated — a forwarded
	// port, a DMZ host, or a Kubernetes hostPort. In those cases inbound to
	// <public ip>:<listen port> arrives untouched and conntrack sends our
	// replies back out of that same port, so the path works both ways and is
	// stable. It was simply never advertised, so no peer ever tried it.
	if m.ip != "" && myUDPPort > 0 && m.port != myUDPPort {
		addAlways(fmt.Sprintf("%s:%d", m.ip, myUDPPort))
	}

	// 5. LAST: predicted ports for a symmetric NAT. These are guesses — most
...
func doHTTPTrackerAnnounce(trackerURL string, infoHash []byte, peerID string, port int) (TrackerResponse, error) {
	...
	// BEP7: advertise our global IPv6 so the tracker lists us in peers6.
...
func doUDPTrackerAnnounce(tracker string, infoHash []byte, peerID string, port int) (TrackerResponse, error) {
	...
	if n > 20 {
		...
		if jsonErr := json.Unmarshal(body, &r2); jsonErr == nil && len(r2.Peers) > 0 {
			...
			// Announced over IPv6: compact 18-byte [ip6][port] entries. Parsing
			// them as 6-byte v4 records shredded every v6 peer — which matters,
			// since v6 endpoints have no NAT and connect directly.
...
// parseCompactPeers6 decodes BEP7 compact IPv6 peers (18 bytes each). Over v6
// there is no NAT, so these endpoints are directly dialable.
...
// (ZeroTier, Tailscale, WireGuard, Docker/bridges, our own tun) that must not
// be used for LAN discovery or as a transport candidate.
func isVirtualInterface(name string) bool {
	n := strings.ToLower(name)
	prefixes := []string{
		"zt", "feth", "tailscale", "utun", "tun", "tap", "ovl", "wg",
...
// (skipping link-local, ULA, loopback). Because IPv6 has no NAT, these are
...
// myUDPPort is our actual UDP listen port (no NAT translation on IPv6).
var myUDPPort int
...
func udpListener(listenPort int) (*net.UDPConn, int, error) {
	// Wildcard bind on "udp" opens a DUAL-STACK socket (v4 + v6) where the OS
	// supports it, so the transport can fall back to IPv6 (no NAT) while the
	// overlay stays IPv4. Incoming v4 peers normalize back to a.b.c.d.
	if !ipv6Enabled {
...
func sendPacket(conn *net.UDPConn, addr *net.UDPAddr, s *session, payload []byte) error {
	// Post-quantum: once the ML-KEM layer is up for this peer, wrap EVERY frame
	// (data, relayed/exit packets, and admin-key control frames) — except the PQ
	// negotiation frames themselves. pqWrap is a no-op until the layer is ready.
...
func handleControl(body []byte, raddr *net.UDPAddr) {
	...
	case 'G':
		...
		return
	case 'D':
		// Admin-signed network policy (post-quantum on/off), applied live.
	...
	case 'A':
		...
		// Only LEARN a route to an ADMITTED peer — an ipLearning entry is what
		...
		// gossip is just a safety refresh).
	...
	case 'C', 'K':
		...
		// AND that a relay path to it exists right now — the only evidence we
		// get for a node holding no direct sessions, since nothing gossips
		// such a node into our roster and ip_learning forgets it between
		// bursts of traffic. Mirrors client/main.go. See heard.go.
...
func loadTrackerList(cfg *ClientConfig) []string {
	// If an admin managed the tracker list, the managed file is authoritative.
	...
	// Always union in the curated defaults so one flaky/dead tracker in the
	// config can never starve discovery. (A mobile app used to ship ONLY
	// tracker.opentrackr.org, which frequently times out — so on iOS the phone
	// found LAN peers but nothing from the tracker swarm. Dedup happens below.)
...
// defaultTrackers is the curated set of public UDP trackers that were actually
// reachable as of this build. Kept in sync with config/trackers.txt (top of
// list). Do NOT re-add the historical openbittorrent / leechers-paradise /
// pomeranian entries — they are dead (NXDOMAIN) and only slow discovery.
...
// without it the peer stays relay-only and never appears in the peer list (the
// "phone on Wi-Fi is missing a peer the Mac/server both see" symptom). We then
// allow one attempt per retry window so a misbehaving router can't churn.
...
func startLocalDiscovery(infoHash []byte, udpPort int, kp keypair, psk []byte) {
	...
	go func() {
		...
		for {
			...
			// per 5s) so discovery converges even when only ONE side's
			// probes get through — e.g. an iPhone that heard our sweep but
			// whose own probes were lost by the AP.
	...
	// Unicast sweep goroutine — the broadcast beacons above never leave an
	// iPhone: iOS 14+ blocks SENDING and RECEIVING UDP broadcast unless the
	// app holds Apple's restricted multicast entitlement
	// (com.apple.developer.networking.multicast). Plain unicast is always
	// allowed, so while we have no established LAN peer we probe every host
	// on our directly-attached /24-or-smaller subnets with the same beacon.
	// A peer that hears it connects back exactly as it would for a broadcast
	// beacon (the listener doesn't care how the datagram arrived), and its
	// handshake reaches our main UDP port as ordinary unicast. This also
	// fixes LANs whose APs filter broadcast frames. ~254 tiny packets per
	// sweep, paced, and only while no LAN peer is up — negligible.
	go func() {
		...
		sweep := func() {
			...
			for pass := 0; pass < 2; pass++ {
				for _, dst := range targets {
					_, _ = ln.WriteToUDP([]byte(beacon), &net.UDPAddr{IP: dst, Port: discPort})
...
// uncached form issues a routing-table syscall and allocates on every call;
// doing that per packet and per comparison stalls the data path. Interface
// addresses change on the order of seconds at worst (a Wi-Fi roam, a cellular
// handover), so a short TTL costs nothing in accuracy.
...
// non-loopback IPv4 interface (e.g. 10.202.2.255 for 10.202.2.203/24). Used as
// a fallback for LAN peer discovery on networks that drop 255.255.255.255.
