...
var (
	...
	// included in join info so a scanned QR configures matching crypto.
	gCipherName string
)
...
func (t *SessionTable) Snapshot() []SessionInfo {
	...
	for _, r := range raws {
		...
		// from a provision (this is the case when a peer's keepalives haven't
		// reached us yet — e.g. behind a Wi-Fi NAT — but the admin's signed
		// provision has: we were showing the right name from the provision but a
		// key-derived IP because this path skipped the provision); then the
		// key-derived fallback.
		...
		if ip == "" {
			if rec, ok := provisions.get(r.key); ok && rec.Address != "" {
				...
				// Teach the routing table this mapping so traffic to the assigned
				// IP actually reaches this peer's endpoint (fixes "can't ping the
				// assigned IP" even though the peer is connected).
				if ip != "" && r.addr != nil {
					ipLearning.Learn(ip, r.addr)
				}
			}
		}
	...
	if ipLearning != nil {
		...
		for _, e := range ipLearning.Entries() {
			...
			// a row without one can only be looked at.
			pubKey := ""
			if key != ([32]byte{}) {
	...
	// A node that can dial out but never be dialled holds zero direct sessions,
	// so nothing gossips it and ip_learning drops it whenever traffic pauses —
	// leaving it reachable but absent from this list. Listed LAST so a direct
	// session or a real learned route always wins the row.
	// Mirrors client/control.go. See heard.go.
	{
		listed := map[string]bool{}
		listedFPs := map[string]bool{}
		for _, si := range out {
...
func startControlServer(socketPath string) {
	...
	mux.HandleFunc("/api/info", func(w http.ResponseWriter, r *http.Request) {
		writeJSON(w, http.StatusOK, map[string]any{
			...
			// How this device's NAT allocates external ports. "symmetric" means
			// peers behind their own NAT cannot punch to this phone and will
			// relay — the one diagnosis the app previously could not show.
			"nat":      natSummary(),
